...
function vector = calculate_recursively(num_of_loops,vector)
%TEST Summary of this function goes here
%   Detailed explanation goes here


    %disp([num2str(num_of_loops)]);
    disp(vector);
    for dd=1:length(mplanred)
        for xx=1:n
            vector = [vector mplanred(dd,xx,3)];
        end
    end
        
    for i = fliplr(1:num_of_loops)
        vector = test(num_of_loops - 1, vector);
    end
